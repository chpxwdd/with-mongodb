...
import {Badge, Button, ButtonGroup, ButtonToolbar, Card, Col, Form, ListGroup, Row, Stack, Table} from 'react-bootstrap';
...
const Page: NextPage<Props> = ({}) => {
    ...
    React.useEffect(() => {fetchStorageFavoritesLeagues()}, [])
    ...
    const leagues = (data: IResponse[], countryId: string): IStorageLeague[] => data.map(item => item.league).filter(item => item.country === countryId)
                ...
                <Row>
                        ...
                        {favoriteLeagues &&
                            <aside>
                                ...
                                <ListGroup className='list-group-flush m-0'>
                                    {Object
                                        .keys(groupCountries(favoriteLeagues))
                                        .map((country_id: string) => countries(favoriteLeagues).find(item => item._id === country_id))
                                            ...
                                            <ListGroup.Item key={country._id} className='p-0' action>
                                                <div className='d-flex justify-content-between py-1'>
                                                    <div className='d-flex mx-2 align-items-center'>
                                                        <Image src={country?.flag || worldFlag}
                                                            width={0} height={0} alt={country.name}
                                                            className='border border-1 border-default rounded rounded-1' style={{objectFit: "contain", width: '20px', height: "auto"}} />
                                                ...
                                                <div>
                                                    <ListGroup className='list-group-flush p-0 mx-0 mt-0 '>
                                                        {
                                                            leagues(favoriteLeagues, country._id).map(league => <LeagueListGroupItem key={league._id} league={league} leaguesApiId={leaguesApiId} onChange={handleChangeLeague} />)
                                                        }
                                                    </ListGroup>
                                                </div>
                                            </ListGroup.Item>
                    ...
                    <Col lg={8} md={6} >
                        <nav className="justify-content-between border shadow-sm p-2 rounded rounded-2 mb-3 bg-light" >
                            <ButtonGroup size='sm'>
                                <Button variant="warning" onClick={(e) => fetchCheckedStandings()} disabled={!leaguesApiId.length}> fetch standing</Button>
                            </ButtonGroup>
                        </nav>
                        {standings && standings.map((standing: IStandingResponse) => <RapidAPILeagueStanding league={standing.league} />)}
                    </Col>
                </Row>
            </StatisticsLayout >
        </RootLayout >
